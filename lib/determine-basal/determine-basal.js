...
function withinISFlimits(liftISF, minISFReduction, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget) {
    ...
    if ( liftISF < minISFReduction ) {
        ...
        isfadaptionreason = "weakest autoISF factor " + round(liftISF,2) + " limited by autoISF_min " + minISFReduction;  // mod V14j
        console.error(isfadaptionreason);
        liftISF = minISFReduction;                                                                             // mod V14j
    } else if ( liftISF > maxISFReduction ) {
        withinlimitsreason = " (lmtd.)"                                                              // mod V14j
        isfadaptionreason = "strongest autoISF factor " + round(liftISF,2) + " limited by autoISF_max " + maxISFReduction;  // mod V14j
        console.error(isfadaptionreason);
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        transreason += "BG-accel: " + round(bg_acce,3) + ", PF-minutes: " + parabola_fit_minutes + ", PF-corr: " + round(parabola_fit_correlation,4) + ", PF-nextDelta: " + convert_bg(parabola_fit_next_delta,profile) + ", PF-lastDelta: " + convert_bg(parabola_fit_last_delta,profile) +  ", regular Delta: " + convert_bg(glucose_status.delta,profile);
...
if ( !profile.use_autoisf ) {
    ...
    if ( loop_wanted_smb=='enforced' ) {                                // go for max impact
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd_averages) {
   ...
   // const average_total_data = tdd_averages.average_total_data;
   const tdd_ytd = tdd_averages.tddYtd;
   const average_7days = tdd_averages.tdd7d;
...
// Cancel high temps (and replace with neutral) or shorten long zero temps for various error conditions
    ...
    var cgmWaitLimit = 44   // is this a useful default?
    ...
    } else if ( bg > 60 && glucose_status.cgmFlatMinutes>cgmWaitLimit && !isSaveCgmSource) {
        if ( glucose_status.last_cal && glucose_status.last_cal < 3 ) {
            rT.reason = "CGM was just calibrated";
        } else {
            rT.reason = "Error: CGM data was suspiciously flat for the past ~" + glucose_status.cgmFlatMinutes + "m";
        }
    }
    //cherry pick from oref upstream dev cb8e94990301277fb1016c778b4e9efa55a6edbc
    if (bg <= 10 || bg === 38 || noise >= 3 || minAgo > 12 || minAgo < -5 || ( bg > 60 && glucose_status.cgmFlatMinutes>cgmWaitLimit ) && !isSaveCgmSource ) {
