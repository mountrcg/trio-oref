...
function loop_smb(profile, iob_data, target_bg) {
    if (profile.use_autoisf && profile.iob_threshold > 0 &&  profile.iob_threshold < iob_data[0].iob) {
        smbreason = ", autoISF-SMB disabled:, IOB: " + round(iob_data[0].iob,2) + ", > threshold: " + profile.iob_threshold + ", maxIOB: " + profile.max_iob;
        console.error(smbreason);
        return "iobTH"
    }
    if (profile.use_autoisf && profile.temptargetSet && profile.enableSMB_EvenOn_OddOff)  {
        var target = convert_bg(target_bg, profile);
...
function withinISFlimits(liftISF, minISFReduction, maxISFReduction, sensitivityRatio) {   // extracted 17.Mar.2022
    ...
    if ( liftISF < minISFReduction ) {
        withinlimitsreason = " (lmtd.)"                                                             // mod V14j
        isfadaptionreason = "weakest ISF factor " + round(liftISF,2) + " limited by autoisf_min " + minISFReduction;  // mod V14j
        console.error(isfadaptionreason);
        liftISF = minISFReduction;                                                                             // mod V14j
    } else if ( liftISF > maxISFReduction ) {
        withinlimitsreason = " (lmtd.)"                                                              // mod V14j
        isfadaptionreason = "strongest ISF factor " + round(liftISF,2) + " limited by autoisf_max " + maxISFReduction;  // mod V14j
        console.error(isfadaptionreason);
    ...
    isfadaptionreason = "final ISF factor " + round(final_ISF,2)  // mod V14j
    console.error(isfadaptionreason);
    return final_ISF;
}

function autoISF(sens, target_bg, profile, glucose_status, meal_data, currentTime, sensitivityRatio, normalTarget)
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    const bg_acceleration = glucose_status.bg_acceleration;
    const parabola_fit_a0 = glucose_status.a_0;
    const parabola_fit_a1 = glucose_status.a_1;
    const parabola_fit_a2 = glucose_status.a_2;
    const dura05 = glucose_status.dura_ISF_minutes;           // mod 7d
    const avg05  = glucose_status.dura_ISF_average;            // mod 7d
    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        isfreason += ", autoISF, disabled with COB";
        return sens;
    } else {
        ...
        if (bg_ISF<1) {
            ...
            if ( acce_ISF>1 ) {                                                                                 // mod V14j
                ...
                } else {isfreason +=  ", bg-ISF Ratio: " + round(liftISF,2)}                                                                                          // mod V14j
            final_ISF = withinISFlimits(liftISF, profile.autoISF_min, profile.autoISF_max, sensitivityRatio);
            // use autoISF on already adjusted profile.sens (via TempTarget) during exercise
            if ((profile.high_temptarget_raises_sensitivity || profile.exercise_mode) && profile.temptargetSet && target_bg > normalTarget) {
                autoISFsens = Math.min(720, round(sens / final_ISF, 1));
            } else {
                autoISFsens =  Math.min(720, round(profile.sens / final_ISF, 1));
            };
            isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + withinlimitsreason + ", final ISF: " + convert_bg(autoISFsens, profile);
            return autoISFsens;
        } else if ( bg_ISF > 1 ) {
            sens_modified = true;
        ...
        if (profile.enablepp_ISF_always || profile.pp_ISF_hours >= (currentTime - meal_data.lastCarbTime) / 1000/3600) {  // corrected logic on 17.Sep.2021
            deltaType = 'pp'
        } else {
            deltaType = 'delta'
        }
        ...
        if ( sens_modified ) {
            ...
            if ( acce_ISF < 1 ) {
                isfadaptionreason = "strongest ISF factor " + round(liftISF,2) + " weakened to " + round(liftISF*acce_ISF,2) + " as bg decelerates already";  // mod V14j
            ...
            final_ISF = withinISFlimits(liftISF, profile.autoISF_min, profile.autoISF_max, sensitivityRatio);
            // take care of exercise mode
            if ((profile.high_temptarget_raises_sensitivity || profile.exercise_mode) && profile.temptargetSet && target_bg > normalTarget) {
                autoISFsens = round(sens / final_ISF, 1);
                console.error("autoISF adjusts sens " + autoISFsens + ", instead of profile.sens "  + profile.sens)
            } else {
                autoISFsens = round(profile.sens / final_ISF, 1);
            };
            isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + withinlimitsreason + ", final ISF: " + convert_bg(autoISFsens, profile);
            //console.error("Inside autoISF: " + isfreason);
            return autoISFsens;
        }
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_daily, tdd_avg) {
        ...
        var quota = 0;
        var tdd_ytd = 0;
        if (tdd_daily.length > 0) {tdd_ytd = tdd_daily[0].TDD};
        let average_7days = tdd_avg.avgTDD7d;
//        if (tdd_avg.length >= 0) {average_7days = tdd_avg[0].avgTDD7d}
    ...
    if (pumpData > 21) {
        ...
        if (weightedAverage > 0 && average_total_data > 0) {
            tddReason = ", TDD: " + round(tdd,1) + " U, Weighted avg: " + round(weightedAverage,1) + " U, Total data avg: " + round(average_total_data,1) + " U" + ", distribution(bolus: " + bolusInsulin.toPrecision(5) + ", temp_basal: " + tempInsulin.toPrecision(5) + ", insulin: " + insulin_;
        } else { tddReason = ", TDD: " + round(tdd,2 )+ ", insulin: " + insulin_; }
...
// Adjust ISF based on sensitivityRatio
    ...
    console.error(" start autoISF");
    console.error("----------------------------------");
    console.error("autoISFtimer start");
    var loop_wanted_smb = loop_smb(profile, iob_data, target_bg);
    var enableSMB = false;
    if (microBolusAllowed && loop_wanted_smb != "oref") {
        ...
        console.error("loopSMB function overriden with autoISF temptarget toggle, enableSMB = " + enableSMB);
       ...
       console.error("loopSMB function returns enableSMB = " + enableSMB);
    }

    //console.error("before autoISF ");
    sens = autoISF(sens, target_bg, profile, glucose_status, meal_data, currentTime, sensitivityRatio, normalTarget);//autoISF
    console.error("autoISFtimer end");
    console.error("----------------------------------");
    console.error(" end autoISF");
    console.error("----------------------------------");
    //console.error("after autoISF ");
...
// Initialize rT (requestedTemp) object. Has to be done after eventualBG is calculated.
    rT = {
        ...
        , 'tick': tick
        , 'eventualBG': convert_bg(eventualBG,profile)
        , 'insulinReq': 0
...
// Print summary variables based on predBGs etc.
    ...
    // with final_ISF ratio being 1 autoISF does not adapt anythin, so regular sensitivityRatio will be applied
    if (final_ISF == 1) {
        rT.auto_ISFratio = round(sensitivityRatio,2);
    } else {rT.auto_ISFratio = round(final_ISF,2)};
    ...
    //console.error(isfreason)
    var smb_ratio = determine_varSMBratio(profile, bg, target_bg, loop_wanted_smb);
    rT.SMBratio = round(smb_ratio,2);
    var SMBdelreason = "SMB Ratio:, " + round(smb_ratio,2) + ", ";
    rT.reason = SMBdelreason + isfreason + ", Standard, COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI+ ", ISF: " + rT.ISF + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
    if (lastCOBpredBG > 0) {
