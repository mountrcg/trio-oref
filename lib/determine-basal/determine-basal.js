...
function loop_smb(profile, iob_data, aimismb) {
    ...
    if ((profile.use_autoisf && profile.temptargetSet && profile.enableSMB_EvenOn_OddOff) || (profile.use_autoisf && profile.min_bg==profile.max_bg && profile.enableSMB_EvenOn_OddOff_always))  {
        ...
        if ( !evenTarget ){
            ...
            console.error("enableSMB_EvenOn_OddOff = " + profile.enableSMB_EvenOn_OddOff + ", enableSMB_EvenOn_OddOff_alway = " + profile.enableSMB_EvenOn_OddOff_always)
...
// Set variables required for evaluating error conditions
    ...
    var insulinRecommended = 0;
    var manualBolusError = 0;
...
// Adjust ISF based on sensitivityRatio
    ...
    var loop_wanted_smb = loop_smb(profile, iob_data, aimismb);
    var enableSMB = true;
    if (microBolusAllowed && loop_wanted_smb != "oref") {
        if ( loop_wanted_smb == "blocked" || loop_wanted_smb == "AIMI B30") {              // otherwise FL switched SMB off
            enableSMB = false;
        }
...
// Print summary variables based on predBGs etc.
    ...
    rT.reason =  B30reason + SMBdelreason + TTreason + isfreason + ", Standard, COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI+ ", ISF: " + rT.ISF + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
...
// Use minGuardBG to prevent overdosing in hypo-risk situations
    ...
    if (enableSMB && minGuardBG < threshold) {
        ...
        manualBolusError = 2;
        insulinRecommended = round((eventualBG - target_bg) / sens, 2);
        enableSMB = false;
    }
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        rT.insulinForManualBolus = insulinRecommended;
        rT.manualBolusErrorString = manualBolusError;
