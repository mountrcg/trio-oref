...
function loop_smb(profile, iob_data) {                  // use odd and even TempTargets to regulate autoISF SMB's and prevent SMB's above IOB threshold
    ...
    if (profile.temptargetSet && profile.enableSMB_EvenOn_OddOff && profile.use_autoisf) {
        var target = convert_bg(profile.target_bg, profile);
        if (profile['out_units'] == "mmol/L") {
            evenTarget = ( round(target*10, 0) %2 == 0 )  //even mmol/l value
        } else {
            evenTarget = ( target %2 == 0 )         //event in mg/dl
        }
        if (!evenTarget) {         // odd number
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        if (bg_ISF<1) {
            ...
            sens =  Math.min(720, round(profile.sens / final_ISF, 1));
            isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + withinlimitsreason + ", final ISF: " + convert_bg(sens, profile);
            return sens;
        } else if ( bg_ISF > 1 ) {
            sens_modified = true;
        ...
        if ( sens_modified ) {
            ...
            final_ISF = withinISFlimits(liftISF, profile.autoISF_min, profile.autoISF_max, sensitivityRatio);
            sens = round(profile.sens / final_ISF, 1)
            isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + withinlimitsreason + ", final ISF: " + convert_bg(sens, profile);
