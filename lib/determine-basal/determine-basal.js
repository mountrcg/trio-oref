...
function loop_smb(profile, iob_data, target_bg) {
    ...
    if ((profile.use_autoisf && profile.temptargetSet && profile.enableSMB_EvenOn_OddOff) || profile.use_autoisf && profile.min_bg==profile.max_bg && profile.enableSMB_EvenOn_OddOff_always)  {
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        if (bg_ISF<1) {
            ...
            isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + withinlimitsreason + ", final ISF: " + convert_bg(profile.sens,profile) + "\u2192" + convert_bg(autoISFsens, profile);
            return autoISFsens;
        } else if ( bg_ISF > 1 ) {
            sens_modified = true;
...
// Adjust ISF based on sensitivityRatio
    ...
    if (microBolusAllowed && loop_wanted_smb != "oref") {
        if ( loop_wanted_smb == "blocked") {              // otherwise FL switched SMB off
            enableSMB = false;
        }
        console.error("loopSMB function overriden with autoISF target toggle, enableSMB = " + enableSMB);
