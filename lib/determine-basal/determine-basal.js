...
function loop_smb(profile, iob_data, target_bg) {
    console.error("Threshold: " + profile.iob_threshold + " IOB: " + round(iob_data[0].iob,2));
...
function autoISF(sens, target_bg, profile, glucose_status, meal_data, currentTime, autosens_data, sensitivityRatio, normalTarget)
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        isfreason += smbreason + ", SMB Delivery Ratio:, " + round(smbRatio,2) + calcreason + ", autoISF";
...
// Adjust ISF based on sensitivityRatio
    ...
    var loop_wants_smb = loop_smb(profile, iob_data, target_bg);
    //console.error("before autoISF ");
    sens = autoISF(sens, target_bg, profile, glucose_status, meal_data, currentTime, autosens_data, sensitivityRatio, normalTarget);//autoISF
...
// Initialize and calculate variables used for predicting BGs
    ...
    var enableSMB = false;
    var loop_wanted_smb = loop_wants_smb;       // variable SMB enabling by autoISF
...
// Print summary variables based on predBGs etc.
    ...
    rT.reason = isfreason + ", Standard, COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI+ ", ISF: " + rT.ISF + ", CR: " + rT.CR + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
    if (lastCOBpredBG > 0) {
