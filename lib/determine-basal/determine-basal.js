...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        if ( bg_ISF < 1 && acce_ISF > 1 ) {                                                                             // mod V14j
            ...
            isfadaptionreason = "bg-ISF adaptation lifted to " + round(bg_ISF,2) + ", as BG accelerates already";   // mod V14j
            bgisfreason = "(lifted by " + round(acce_ISF,2) + ")";
            console.error(isfadaptionreason);
            }                                                                                                       // mod V14j
        var liftISF = 1;
        if (bg_ISF < 1) {                                                                                                      // mod V14j
            liftISF = Math.min(bg_ISF, acce_ISF);
            if ( liftISF < profile.autoisf_min ) {                                                                               // mod V14j
                isfadaptionreason = "final ISF factor " + round(liftISF,2) + " limited by autoisf_min " + profile.autoisf_min;  // mod V14j
                console.error(isfadaptionreason);
        ...
        {     // corrected logic on 17.Sep.2021
        ...
        isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(liftISF,2) + ", final ISF: " + convert_bg(sens,profile);
