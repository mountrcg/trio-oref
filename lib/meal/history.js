...
function findMealInputs (inputs) {
    ...
    for (var i=0; i < carbHistory.length; i++) {
        ...
        if (!arrayHasElementWithSameTimestampAndProperty(mealInputs,current.actualDate,"carbs")) {
    ...
    for (i=0; i < pumpHistory.length; i++) {
        ...
        } else if ((current._type === "Meal Bolus" || current._type === "Correction Bolus" || current._type === "Snack Bolus" || current._type === "Bolus Wizard" || current._type === "Carb Correction") && current.actualDate) {
